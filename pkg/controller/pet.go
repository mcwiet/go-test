...
type PetService interface {
	...
	UpdateOwner(requestorId string, id string, owner string) (model.Pet, error)
}
...
func (c *PetController) HandleUpdateOwner(request Request) Response {
	...
	updatedPet, err := c.petService.UpdateOwner(request.Identity.Claims.Username, input.Id, input.Owner)
